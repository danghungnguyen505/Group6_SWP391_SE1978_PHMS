<%--
    Document   : change-password.jsp
    Created on : Jan 22, 2026, 8:14:35 AM
    Author     : Nguyen Dang Hung
--%>


<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
<fmt:setBundle basename="util.messages" />
<!DOCTYPE html>
<html lang="${currentLanguage}">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title><fmt:message key="auth.change.title"/> - <fmt:message key="app.brand"/></title>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700;900&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="${pageContext.request.contextPath}/assets/css/base.css">
    <link rel="stylesheet" href="${pageContext.request.contextPath}/assets/css/components.css">
    <link rel="stylesheet" href="${pageContext.request.contextPath}/assets/css/pages/change-password.css">
</head>
<body>
    <%-- Language Switcher --%>
    <div class="language-switcher-fixed">
        <a href="${pageContext.request.contextPath}/language?lang=vi&redirect=/change-password"
           class="lang-btn ${currentLanguage == 'vi' ? 'active' : ''}">VN</a>
        <a href="${pageContext.request.contextPath}/language?lang=en&redirect=/change-password"
           class="lang-btn ${currentLanguage == 'en' ? 'active' : ''}">EN</a>
    </div>

    <div class="back-home-floating">
        <a href="${pageContext.request.contextPath}/home" class="back-home-btn">
            <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
            <fmt:message key="auth.change.backToProfile"/>
        </a>
    </div>
    <div class="change-pass-wrapper">
        <div class="change-pass-card">
            <div class="change-pass-header">
                <div class="logo-icon-box">
                    <svg width="24" height="24" fill="none" stroke="white" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                    </svg>
                </div>
                <h1 class="change-pass-title"><fmt:message key="auth.change.securityTitle"/></h1>
                <p class="change-pass-subtitle"><fmt:message key="auth.change.securityDesc"/></p>
            </div>

            <c:if test="${not empty error}">
                <div class="alert-box alert-error">
                    <svg width="20" height="20" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <span>${error}</span>
                </div>
            </c:if>

            <c:if test="${not empty message}">
                <div class="alert-box alert-success">
                    <svg width="20" height="20" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                    </svg>
                    <span>${message}</span>
                </div>
            </c:if>

            <div id="clientError" class="alert-box alert-error" style="display: none;">
                <svg width="20" height="20" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <span id="errorText"></span>
            </div>

            <form action="change-password" method="post" onsubmit="return validatePasswords()">
                <div class="form-group">
                    <label class="label-micro"><fmt:message key="auth.change.oldPassword"/></label>
                    <div class="input-wrapper">
                        <div class="input-icon">
                            <svg width="20" height="20" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z" />
                            </svg>
                        </div>
                        <input type="password" name="oldPass" required class="form-input" placeholder="<fmt:message key='auth.change.oldPasswordPlaceholder'/>">
                    </div>
                </div>

                <div class="form-group">
                    <label class="label-micro"><fmt:message key="auth.change.new"/></label>
                    <div class="input-wrapper">
                        <div class="input-icon">
                            <svg width="20" height="20" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                            </svg>
                        </div>
                        <input type="password" id="newPass" name="newPass" required minlength="6" class="form-input" placeholder="<fmt:message key='auth.change.newPasswordPlaceholder'/>">
                    </div>
                </div>

                <div class="form-group">
                    <label class="label-micro"><fmt:message key="auth.change.confirm"/></label>
                    <div class="input-wrapper">
                        <div class="input-icon">
                            <svg width="20" height="20" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                            </svg>
                        </div>
                        <input type="password" id="confirmPass" name="confirmPass" required class="form-input" placeholder="<fmt:message key='auth.change.confirmPlaceholder'/>">
                    </div>
                </div>

                <button type="submit" class="btn-submit">
                    <fmt:message key="auth.change.update"/>
                </button>

                <a href="${pageContext.request.contextPath}/profile" class="back-link">
                    <fmt:message key="auth.change.backToProfile"/>
                </a>
            </form>
        </div>
    </div>

    <script>
        function validatePasswords() {
            const newPass = document.getElementById("newPass").value;
            const confirmPass = document.getElementById("confirmPass").value;
            const errorDiv = document.getElementById("clientError");
            const errorText = document.getElementById("errorText");

            if (newPass !== confirmPass) {
                errorText.innerText = "<fmt:message key='auth.change.mismatch'/>";
                errorDiv.style.display = "flex";
                return false;
            }

            errorDiv.style.display = "none";
            return true;
        }

        document.getElementById("confirmPass").addEventListener("input", function() {
            document.getElementById("clientError").style.display = "none";
        });
    </script>
</body>
</html>
