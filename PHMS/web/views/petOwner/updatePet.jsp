...
<head>
    ...
    <title>${L == 'en' ? 'Update Pet Profile' : 'Cập nhật hồ sơ thú cưng'} - VetCare Pro</title>
    ...
    <main class="main-content">
        ...
        <div class="page-header">
            <div class="page-title">
                <h1>${L == 'en' ? 'Update Pet Profile' : 'Cập nhật hồ sơ thú cưng'}</h1>
                <p>${L == 'en' ? 'Edit your pet information.' : 'Chỉnh sửa thông tin thú cưng.'}</p>
            </div>
        ...
        <c:if test="${empty pet}">
            <div class="alert alert-warning">
                ...
                ${L == 'en' ? 'Pet information not found.' : 'Không tìm thấy thông tin thú cưng.'}
            </div>
        </c:if>

        <c:if test="${not empty pet}">
            <div class="card update-pet-card">
                    ...
                    <form action="${pageContext.request.contextPath}/pet/update" method="post">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" class="form-control" name="species" value="${pet.species}" maxlength="50"
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" class="form-control" name="breed" value="${pet.breed}" maxlength="100"
                                       placeholder="${L == 'en' ? 'e.g. Golden Retriever' : 'Ví dụ: Golden Retriever'}" required>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select" name="gender" required>
                                    <option value="" disabled>${L == 'en' ? '-- Select gender --' : '-- Chọn giới tính --'}</option>
                                    <option value="Male" ${pet.gender == 'Male' ? 'selected' : ''}>${L == 'en' ? 'Male' : 'Đực'}</option>
                                    <option value="Female" ${pet.gender == 'Female' ? 'selected' : ''}>${L == 'en' ? 'Female' : 'Cái'}</option>
                                </select>
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label fw-bold">${t_birth_date} <span class="required-mark">*</span></label>
                                <input type="date" class="form-control" name="birthDate" value="${not empty rawDob ? rawDob : pet.birthDate}" required>
                            </div>
                        </div>
                        ...
                        <div class="mb-4">
                            <label class="form-label fw-bold">${L == 'en' ? 'History Summary' : 'Tóm tắt tiền sử bệnh'}</label>
                            <textarea class="form-control" name="history" rows="4" maxlength="2000"
                                      placeholder="${L == 'en' ? 'Medical history notes...' : 'Ghi chú tiền sử bệnh...'}">${pet.historySummary}</textarea>
                            <div class="form-text text-muted">${L == 'en' ? 'Max 2000 characters.' : 'Tối đa 2000 ký tự.'}</div>
                        </div>

                        <div class="d-grid">
                            <button class="btn btn-primary btn-lg fw-bold" type="submit">
                                <i class="fa-solid fa-floppy-disk me-2"></i> ${L == 'en' ? 'Save Changes' : 'Lưu thay đổi'}
                            </button>
                        </div>
                    </form>
                </div>
